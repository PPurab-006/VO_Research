#!/usr/bin/env python3
"""
Environment Sanity Check & Hover Test Script for exploration_complex/3dmap.world (Candidate #2)
Tests vehicle spawn, hovering at X=19.0, Y=-15.0, Z=2.5m, camera feed, and VO pipeline.
...
class CameraDumper(Node):
    def __init__(self, camera_topic, output_dir):
        super().__init__('sanity_3dmap_camera_dumper')
    ...
    def image_cb(self, msg):
        if self.count < 5:
            try:
                ...
                fn = os.path.join(self.output_dir, f"3dmap_frame_{self.count+1:02d}.png")
...
def main():
    print("============================================================")
    print("ENVIRONMENT SANITY CHECK — exploration_complex/3dmap.world")
    print("============================================================")
    ...
    print(f"[1/6] Launching PX4 SITL with 3dmap world at spawn pose {SPAWN_POSE}...")
    ...
    vo_cmd = [
        ...
        "--output-dir", "results",
        "--output-filename", "sanity_3dmap_vo.csv",
        "--max-frames", "1000",
        "--mode", "klt"
    ]
    ...
    print("[6/6] Hovering for 12 seconds at X=19.0, Y=-15.0, Z=2.5m...")
    start_t = time.time()
    ...
    kill_sim()
    print("[COMPLETE] 3dmap sanity check flight finished successfully.")
