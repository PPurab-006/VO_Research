#!/usr/bin/env python3
"""
Environment Sanity Check & Hover Test Script for agriculture.world
Tests vehicle spawn, hovering at X=-30.0, Y=-30.0, Z=2.5m above ground, camera feed, and VO pipeline.
...
class CameraDumper(Node):
    def __init__(self, camera_topic, output_dir):
        super().__init__('sanity_agriculture_camera_dumper')
    ...
    def image_cb(self, msg):
        if self.count < 5:
            try:
                ...
                fn = os.path.join(self.output_dir, f"agriculture_frame_{self.count+1:02d}.png")
...
def main():
    print("============================================================")
    print("ENVIRONMENT SANITY CHECK — agriculture.world")
    print("============================================================")
    ...
    gt_cmd = [
        ...
        "--output-dir", "results",
        "--filename", "sanity_agriculture_gt.csv",
        "--max-samples", "1500"
    ]
    ...
    vo_cmd = [
        ...
        "--output-dir", "results",
        "--output-filename", "sanity_agriculture_vo.csv",
        "--max-frames", "1000",
        "--mode", "klt"
    ]
    ...
    print("[6/6] Hovering for 12 seconds at X=-30.0, Y=-30.0, Z=2.5m...")
    start_t = time.time()
    ...
    kill_sim()
    print("[COMPLETE] agriculture sanity check flight finished successfully.")
