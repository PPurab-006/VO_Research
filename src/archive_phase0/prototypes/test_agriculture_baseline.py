#!/usr/bin/env python3
"""
Final Environment & Spawn Baseline Test Script for agriculture.world
Target hover: X = -30.0m, Y = -30.0m, Z = 2.5m above local terrain (-2.657m ground).
...
SPAWN_POSE = "-30.0,-30.0,-2.457,0,0,0"

class CameraDumper(Node):
    def __init__(self, camera_topic, output_dir):
        super().__init__('baseline_agriculture_camera_dumper')
    ...
    def image_cb(self, msg):
        if self.count < 5:
            try:
                ...
                fn = os.path.join(self.output_dir, f"baseline_agri_frame_{self.count+1:02d}.png")
...
def main():
    print("============================================================")
    print("FINAL ENVIRONMENT + SPAWN BASELINE TEST — agriculture.world")
    print("============================================================")
    ...
    print(f"[1/6] Launching PX4 SITL with agriculture world at spawn pose {SPAWN_POSE}...")
    ...
    vo_cmd = [
        ...
        "--output-dir", "results",
        "--output-filename", "baseline_agri_vo.csv",
        "--max-frames", "1000",
        "--mode", "klt"
    ]
    ...
    master.wait_heartbeat(timeout=25)
    ...
    print("      Sending Takeoff to 2.5m above local ground...")
    master.mav.command_long_send(
        master.target_system, master.target_component,
    ...
    print("[6/6] Hovering for 12 seconds at X=-30.0, Y=-30.0, Z=2.5m above terrain...")
    start_t = time.time()
    ...
    kill_sim()
    print("[COMPLETE] Agriculture baseline hover check finished.")
