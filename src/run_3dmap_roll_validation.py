#!/usr/bin/env python3
"""
Orchestration Script for Single 10-Degree Roll Validation Flight in 3dmap.world (Phase 0E Discriminative Experiment)

Runs one 10-degree roll validation flight in exploration_complex/3dmap.world with GPU offload.
Hover Target: X = 19.0 m, Y = -15.0 m, Z = 2.5 m above local terrain (0.0 m ground level -> hover Z = 2.50 m).
Ensures clean 2-stage process lifecycle cleanup (SIGTERM -> SIGKILL).
Saves GT and VO logs to results/roll_3dmap_gt.csv, results/roll_3dmap_vo.csv, and results/roll_3dmap_telemetry.csv.
"""
...
def main():
    print("==========================================================================================")
    print("PHASE 0E — 10-DEGREE ROLL VALIDATION FLIGHT EXPERIMENT (3dmap.world DISCRIMINATIVE TEST)")
    print("==========================================================================================")
    ...
    subprocess.run(["ln", "-sf", f"{WORLD_DIR}/3dmap.world", f"{PX4_DIR}/Tools/simulation/gz/worlds/default.sdf"])
    ...
    print(f"[1/5] Launching PX4 SITL (World: 3dmap.world, Spawn: {SPAWN_POSE})...")
    ...
    gt_csv = "results/roll_3dmap_gt.csv"
    vo_csv = "results/roll_3dmap_vo.csv"
    ...
    vo_cmd = [
        ...
        "--output-dir", "results",
        "--output-filename", "roll_3dmap_vo.csv",
        "--max-frames", "2000",
        "--mode", "klt"
    ]
    ...
    # 5. Execute 10-degree roll validation flight in 3dmap.world
    ...
    fly_cmd = [
        ...
        "--target-y", "0.0",
        "--target-z", "2.50",
        "--telem-filename", "roll_3dmap_telemetry.csv"
    ]
    ...
    kill_all_sim_processes()
    print("[COMPLETE] 10-degree Roll Validation flight in 3dmap.world finished successfully.\n")
