#!/usr/bin/env python3
"""
Recording Orchestration Script for Phase 2B Candidate 4 Corrected (R3)

Trajectory: Candidate 4 (Yaw Sweep + Sustained Forward Translation - MASK 504 with Rate Feedforward)
  - Family: F9 (Level 2)
  - Duration: 20.0s after takeoff readiness
  - World: agriculture.world
  - Output Dataset: results/datasets/phase2b_candidate4_L2_R3
...
def record_candidate4_corrected_r3():
    run_id = "phase2b_candidate4_L2_R3"
    dataset_dir = f"results/datasets/{run_id}"

    print("==========================================================================")
    print("PHASE 2B DATASET RECORDER — Candidate 4 Corrected (Mask 504 Rate Feedforward, R3)")
    print("==========================================================================")
    ...
    # 5. Launch Motion Generator (F9 L2 = Yaw Sweep + Translation, mask 504)
    ...
    print("\n[CLEANUP] Wrapping up recording and terminating simulation processes...")
    rec_proc.terminate()
    try:
        rec_proc.wait(timeout=3.0)
    except Exception:
        rec_proc.kill()
    ...
    px4_proc.terminate()
    kill_all_sim_processes()
    print(f"[COMPLETE] Dataset saved to: {dataset_dir}")


if __name__ == "__main__":
    record_candidate4_corrected_r3()
