#!/usr/bin/env python3
"""
Orchestration Script to Record ONE Fresh Dataset for Phase 2A (F5 L2: Pitch + Translation)

Steps:
1. Launches PX4 SITL & Gazebo in agriculture.world.
2. Launches ros_gz_bridge.
3. Launches record_camera_dataset.py to capture 1280x960 PNG frames & ~50 Hz GT attitude.
4. Executes fly_phase1_motion.py (--family F5 --severity 2).
5. Cleanly terminates background processes and verifies dataset integrity.
"""
...
def record_dataset(family='F5', severity=2, duration=20.0, run_id='phase2a_F5_L2'):
    dataset_dir = f"results/datasets/{run_id}"

    print("==========================================================================")
    print(f"PHASE 2A SINGLE DATASET RECORDER — Family: {family}, Level: L{severity}")
    print("==========================================================================")
    ...
    # 5. Launch Motion Generator
    print(f"[5/5] Executing Motion Profile '{family}' L{severity} for {duration:.1f}s...")
    motion_cmd = [
    ...
    kill_all_sim_processes()
    print(f"\n>>> SINGLE DATASET '{run_id}' RECORDING COMPLETE! <<<")


def main():
    parser = argparse.ArgumentParser(description="Record Single Trajectory Dataset for Phase 2A")
    parser.add_argument('--family', type=str, default='F5', help="Motion family ID (e.g. F5, F4, F6)")
    ...
    parser.add_argument('--run-id', type=str, default='phase2a_F5_L2', help="Dataset run ID")
    args = parser.parse_args()

    record_dataset(family=args.family, severity=args.severity, duration=args.duration, run_id=args.run_id)
